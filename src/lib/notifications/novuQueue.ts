...
/**
 * Novu queue processor - replaces OneSignal queue
 * Handles fetching queued notifications, applying user preferences,
 * triggering Novu workflows, and marking processed/failed/skipped notifications
 */
export async function processNovuQueue(options: ProcessQueueOptions = {}): Promise<ProcessQueueResult> {
  ...
  for (const [userId, userNotifications] of Object.entries(notificationsByUser)) {
    try {
      ...
      // Process each notification via Novu workflows
      for (const notification of notificationsToSend) {
        try {
          // Map notification type to Novu workflow ID
          const workflowId = getWorkflowIdForNotificationType(notification.type);
          ...
          const result = await NovuService.triggerWorkflow(workflowId, userId, payload);
      ...
      // Mark skipped notifications as sent (so they don't get processed again)
...
/**
 * Map notification types to Novu workflow IDs
 * These should match the workflows created in your Novu dashboard
 */
function getWorkflowIdForNotificationType(notificationType: string): string {
  ...
  return workflowMap[notificationType] || 'default_notification';
}
