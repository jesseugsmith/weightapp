...
/**
 * Start pending competitions
 * 
 * GET: Vercel Cron Job (runs daily at midnight)
...
async function startPendingCompetitions() {
  console.log('\n🌅 Start Pending Competitions - Processing');
  ...
  console.log(`\n✅ Job complete: ${results.started} started, ${results.failed} failed`);
...
export async function GET(request: NextRequest) {
  console.log('\n🌅 Cron: Start Pending Competitions - Triggered via GET');
  ...
  try {
    ...
    const result = await startPendingCompetitions();
...
export async function POST(request: NextRequest) {
  console.log('\n🌅 Admin: Start Pending Competitions - Triggered via POST');
  ...
  try {
    ...
    console.error('❌ Error in start-competitions admin trigger:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
